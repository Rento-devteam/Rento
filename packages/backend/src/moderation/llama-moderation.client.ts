...
export class LlamaModerationClient implements OnModuleInit {
  ...
  private buildPrompt(input: {
    ...
    const phaseHint =
      input.phase === 'publish'
        ? 'Publish: block only clear abuse or totally unusable text; otherwise allow or warn.'
        : 'Draft: default to allow when the text is readable and about renting one item.';
    return [
      'You are a moderator for a rental marketplace (items for rent). Reply with JSON only — no markdown, no extra text.',
      ...
      '}',
      '',
      'ALLOW (status "allow", all flags false) when:',
      '- The title and description are about ONE item/service for rent, in Russian and/or English.',
      '- Text has a clear topic: what it is, condition, price/terms, pickup — even as bullets or short lines.',
      '- There are minor typos, casual tone, marketing phrases, or lines like "Бренд:", "Год:", "Состояние:" — that is normal.',
      '- Placeholders in brackets [like this] or fill-in hints for the owner are OK.',
      '',
      'BLOCK (status "block") ONLY when:',
      '- Clear profanity, slurs, hate, sexual solicitation, or threats.',
      ...
      '- profanity: true ONLY for explicit slurs/profanity, NOT for normal words or typos.',
      '- gibberish: true ONLY for unreadable noise, NOT for coherent text with spelling mistakes.',
      '- spamLike: true ONLY for link dumps / unrelated promotion.',
      'If status is "allow", all flags MUST be false.',
      '',
      'confidence = how sure you are about status/flags, NOT writing quality. Good rental copy → allow with 0.9–1.0.',
      phaseHint,
