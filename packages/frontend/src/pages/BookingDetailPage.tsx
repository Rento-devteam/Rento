...
import { BookingContractSection } from "../bookings/BookingContractSection";
...
export function BookingDetailPage() {
  ...
  const loadBooking = useCallback(async () => {
    ...
    try {
      ...
      setBooking(null);
      setError(
        err instanceof ApiError ? err.message : "Не удалось загрузить сделку",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirmReturn = async () => {
    ...
    try {
      ...
      if (updated.status === "COMPLETED") {
        ...
        setReturnNotice(
          "Подтверждение отправлено. Ожидаем подтверждение второй стороны.",
        );
      }
    } catch (err: unknown) {
      setReturnError(
        err instanceof ApiError
          ? err.message
          : "Не удалось подтвердить завершение аренды",
      );
    } finally {
  ...
  return (
    <main className="bookings-page">
      <div className="container bookings-page__inner">
        ...
        {user ? (
          <BookingContractSection
            booking={booking}
            currentUserDisplay={
              user.fullName?.trim() || user.email || "Участник сделки"
            }
            initialModalOpen={contractPulse}
            onInitialModalOpenHandled={() => setContractPulse(false)}
          />
        ) : null}
        ...
        {canCancelBooking ? (
          ...
          >
            <h2
              ...
              className="bookings-page__panel-title"
            >
              Отмена аренды
            </h2>
        ...
        {booking.status === "CONFIRMED" || booking.status === "ACTIVE" ? (
          <div
            ...
            style={{ marginTop: "var(--sp-4)" }}
          >
            Средства заблокированы на карте арендатора (Escrow). Календарь
            объявления обновлён на стороне сервера.
          </div>
        ) : null}
        ...
        {canConfirmReturn ? (
          ...
          >
            ...
            {returnError ? (
              <div className="alert alert--error">{returnError}</div>
            ) : null}
            {returnNotice ? (
              <div className="alert alert--success">{returnNotice}</div>
            ) : null}
            <button
              type="button"
              className="btn btn--brand"
        ...
        {booking.status === "COMPLETED" ? (
          <section
            className="bookings-page__panel"
            aria-labelledby="deposit-release-title"
          >
            <h2
              id="deposit-release-title"
              className="bookings-page__panel-title"
            >
              Возврат залога
            </h2>
            {booking.settlementStatus === "SETTLED" ? (
              ...
              <div className="alert alert--error">
                Расчёт ещё не завершён (статус:{" "}
                {bookingSettlementStatusLabelRu(booking.settlementStatus)}
            ...
            <p
              className="bookings-page__fineprint"
              style={{ marginTop: "var(--sp-3)" }}
            >
              Перевод и разблокировка средств обычно выполняются в течение 24
              часов после закрытия сделки.
              {depositReleaseDeadline
                ? ` Ожидаемый срок: до ${formatDateTimeRu(depositReleaseDeadline)}.`
                : ""}
            </p>
          </section>
        ) : null}
        ...
        {showRetry ? (
          <section
            className="bookings-page__panel"
            aria-labelledby="retry-pay-title"
          >
            <h2 id="retry-pay-title" className="bookings-page__panel-title">
              Повторить блокировку с другой картой
            </h2>
            <p className="bookings-page__fineprint">
              Выберите привязанную карту. При необходимости укажите демо-баланс
              для заглушки холда.
            </p>
            <ul
            ...
            {retryError ? (
              <div className="alert alert--error">{retryError}</div>
            ) : null}
            <button
              type="button"
              className="btn btn--brand"
