...
const ownerListingStub = { ownerId: "owner-user-1" } as Pick<
  IListing,
  "ownerId"
> as IListing;
...
describe("ManageCalendarPage", () => {
  beforeEach(() => {
    ...
    getOwnedListingForEditMock.mockRejectedValue(
  ...
  it("loads calendar and renders legend", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: /календарь доступности/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: /выйти из календаря/i }),
    ...
    await waitFor(() => {
      ...
      expect(getCalendarMock).toHaveBeenCalledWith(
        "listing-1",
        expect.any(String),
        expect.any(String),
      );
  ...
  it("blocks selected range when availability is free", async () => {
    ...
    const [listingId, startDate, endDate, , accessToken] =
      blockDatesMock.mock.calls[0];
  ...
  it("does not offer block actions when the viewer is not the listing owner", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: /заблокировать/i }),
