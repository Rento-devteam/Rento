...
export function ManageCalendarPage() {
  ...
  useEffect(() => {
    ...
    async function loadListingMeta() {
      ...
      try {
        if (accessToken) {
          try {
            const owned = await getOwnedListingForEdit(
              calendarListingId,
              accessToken,
            );
            if (!cancelled) {
        ...
        if (!cancelled) {
          ...
          setListingMetaError(
            getErrorMessage(err, "Не удалось загрузить объявление"),
          );
        }
      }
    }
  ...
  const handleBlock = async () => {
    ...
    try {
      ...
      if (!available && conflicts.some((c) => c.status === "BOOKED")) {
        setConflictWarning(
          "Диапазон содержит даты с активными бронированиями. Блокировка/разблокировка может повлиять на сделки.",
        );
      ...
      if (err instanceof ApiError && err.status === 409) {
        setConflictWarning(
          "Диапазон содержит активные сделки. Разблокировка невозможна до завершения сделок.",
        );
      } else {
  ...
  const handleUnblock = async (force = false, cancelBookings = false) => {
    ...
    try {
      ...
      if (err instanceof ApiError && err.status === 409) {
        setConflictWarning(
          "Выбранные даты содержат активные бронирования. Разблокировка отменит существующие сделки. Продолжить?",
        );
      } else {
      ...
      <div style={{ maxWidth: 800, margin: "0 auto" }}>
        ...
        <div className="calendar-legend">
          <div className="legend-item">
            <span className="legend-color legend-color--free"></span> Свободно
          </div>
          <div className="legend-item">
            <span className="legend-color legend-color--booked"></span> Занято
            (Сделка)
          </div>
          <div className="legend-item">
            <span className="legend-color legend-color--blocked"></span>{" "}
            Заблокировано
          </div>
        </div>
        ...
        {!listingMetaError && listingOwnerId && user && !isOwner ? (
          <p
            className="listing-booking-modal__fineprint"
            style={{ marginBottom: "var(--sp-4)" }}
          >
        ...
        <div className="calendar-card">
          ...
          <div className="calendar-grid">
            {gridDays.map((date, i) => {
              ...
              if (status === "BLOCKED_BY_OWNER" || status === "MAINTENANCE")
                statusClass = "calendar-day--blocked";
        ...
        {isOwner && selectStart ? (
          <div className="calendar-actions">
            <h3 className="calendar-actions__title">
              Выбран диапазон:{" "}
              {parseISODate(selectStart).toLocaleDateString("ru-RU")} —{" "}
              {parseISODate(selectEnd ?? selectStart).toLocaleDateString(
                "ru-RU",
              )}
            </h3>
