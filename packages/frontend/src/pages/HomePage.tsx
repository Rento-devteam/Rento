...
export function HomePage() {
  ...
  const loadCatalog = useCallback(
    async (overrides?: { categoryId?: string }) => {
      ...
      try {
        ...
        setCategories((prev) => {
          ...
          for (const listing of response.results)
            map.set(listing.category.id, listing.category);
          return [...map.values()];
        });
      } catch (err) {
        setError(
    ...
    <main>
      <section className="container hero" aria-label="Поиск по каталогу">
        ...
        <p className="hero__subtitle">
          Найдите то, что нужно, на час, день или месяц. Поиск по названию и
          фильтрация по цене — в одном окне.
        </p>
          ...
          {sections.map((tile) => (
            <button
              ...
              className={`section-tile${
                tile.categoryId && tile.categoryId === categoryId
                  ? " section-tile--active"
                  : ""
              }`}
      ...
      {filterOpen ? (
        ...
        >
          <section
            className="filters-modal__dialog"
          ...
          <div className="catalog__grid">
            ...
            {!loading && !error && visibleItems.length === 0 ? (
              <div className="status catalog__grid-span">
                По запросу ничего не найдено. Попробуйте изменить параметры
                фильтрации.
              </div>
            ) : null}
          </div>
...
function Card({ item }: { item: IListing }) {
  ...
  const locationLine =
    item.addressText?.trim() || displayParts.address?.trim() || null;
  const conditionRu = listingConditionLabelRu(displayParts.condition);
  const cardDescription = [
    conditionRu ? `Состояние: ${conditionRu}` : null,
    displayParts.description,
  ]
    .filter((part): part is string =>
      Boolean(part && part.trim() && part.trim() !== "—"),
    )
    .join(". ");
  return (
    <article className="card">
      ...
      <div className="card__image-wrap">
        {cover ? (
          <img src={cover} alt={item.title} className="card__image" />
        ) : null}
      ...
      <div className="card__body">
        ...
        <div className="card__prices">
          <span className="card__price-main">
            {formatListingRentalPriceRu(item.rentalPrice, item.rentalPeriod)}
          </span>
        </div>
      </div>
