...
const deleteListingMock = vi.hoisted(() =>
...
describe("ProfilePage", () => {
  ...
  it("renders user info and trust index", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: /Индекс доверия/i }),
    ).toBeInTheDocument();
  ...
  it("saves profile when form is submitted", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /Редактировать данные профиля/i }),
    );
    ...
    await waitFor(() => {
      expect(
        screen.queryByRole("dialog", { name: /Личные данные/i }),
  ...
  it("opens delete listing modal and deletes on confirm", async () => {
    ...
    const listingsSection = screen
      .getByRole("heading", { name: /Мои объявления/i })
      .closest("section");
    expect(listingsSection).toBeTruthy();
    const buttons = (listingsSection as HTMLElement).querySelectorAll(
      "button.btn",
    );
    const deleteListingBtn = [...buttons].find(
    ...
    expect(
      await screen.findByRole("heading", { name: /Удалить объявление\?/i }),
    ).toBeInTheDocument();
    expect(screen.getAllByText(/Моя палатка/i).length).toBeGreaterThanOrEqual(
      1,
    );
  ...
  it("attaches card from gateway token", async () => {
    ...
    await user.type(
      screen.getByLabelText(/Токен карты из шлюза/i),
      "pm_token_escrow_1",
    );
    ...
    expect(
      await screen.findByText(/Карта успешно привязана/i),
    ).toBeInTheDocument();
