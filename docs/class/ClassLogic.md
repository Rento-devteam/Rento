...
*Airbnb-подобная система с эскроу, модерацией (пост-MVP) и ИИ-функциями (ARS)*

**MVP:** публикация объявления сразу в `ACTIVE` после заполнения требований (в т.ч. фото), без обязательной модерации перед выходом в ленту. Очередь модерации и связанные сущности — **будущий этап**.
...
| **Moderation**  | `Dispute`, `Report`, `Moderator`, `ModerationLog`  | Модерация и споры *(пост-MVP / расширение)* |
...
- Залог: `depositAmount`
- Статус (MVP): `DRAFT` → `ACTIVE` (после публикации, при выполнении условий, например наличие фото) → `ARCHIVED`
- Статус (пост-MVP, опционально): возможен этап `PENDING_MODERATION` перед `ACTIVE`, если включена пре-модерация
